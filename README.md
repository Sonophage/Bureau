...
### 2.16.0
- **Mobile: the line you type on no longer sits under the pane's bottom edge.** While typing on a phone, the edited line was allowed to settle 20px off the bottom of the editor, which parks it under the pane's frame and chrome so the text you are writing reads as dimmed. That 20px is Obsidian's own `--autoscroll-padding`, the only CSS-side lever there is: CodeMirror's cursor margin is a JavaScript facet a theme cannot reach, and padding on the scroller does not move the limit because the scroller's `clientHeight` already includes it. Bureau now raises that floor, and exposes it as *Caret clearance from the bottom edge* (20-240px, default 64) in a new **Mobile** section, because how much clearance a screen needs depends on its size and on whether the mobile toolbar is showing.
- **Mobile: Atmosphere can now be switched on.** A phone got no film grain, glow, halo, vignette, panel grain, CRT scanlines or card bottom-fade, which left eight existing Atmosphere and CRT sliders inert there with nothing in the panel to say why. A new *Atmosphere on mobile* toggle (default off, so the baseline is unchanged) releases all of them at their current settings. They are full-screen composited layers and the reason they were held off is frame rate on older devices, not looks, so it is opt-in. Gating them also lifted those rules by one class, which is the specificity they turned out to need: on mobile the light-mode panel grain, and the CRT scanline layers under *Inverted editor* or the per-mode editor flip, were quietly out-specifying the very rules meant to switch them off, so they were still painting on a phone.
- **Mobile: touch target size and file-tree row padding are now dials.** Both were hard-coded — 40px minimum tap targets and 9px of row padding — with no way to raise them, and WCAG 2.5.5 asks for 44. *Touch target size* (32-56px) sizes the header icons, tab headers, nav buttons, menu items and suggestion rows; *File-tree row padding* (4-20px) sizes the explorer rows. The tree keeps using padding rather than a minimum height on purpose: forcing a height there re-formats Obsidian's windowed list and, on older Android, left the tree blank until it was tapped.
- **Mobile: the pane's rounded corners are properly gone.** The mobile baseline sets `border-radius: 0` for the flat phone look, but the card's `clip-path` rounds every descendant regardless of the radius, so a phone kept the 12px rounded corners anyway. The clip is now cleared on mobile as well.
